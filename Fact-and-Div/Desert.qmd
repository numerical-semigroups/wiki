...
Let $S$ be a [numerical semigroup](../Basic-concepts/Numerical-semigroup.qmd) and $G(S)$ the set of [gaps](../Basic-concepts/Gap.qmd) of $S$. A subset $A \subseteq \mathbb{N}$ is a desert of $S$ if $A = \mathbb{Z}^{< 0}$ or $A \subseteq G(S)$ and $\min A - 1, \max A + 1 \in S$. Equivalently, $A$ is a desert of $S$ if $A$ is a maximal interval of gaps of $S$ (counting also the set of all negative integers).

It can be proven that the number of deserts is $|Ap(S, 1)|$, where $Ap(S,1)$ denotes the [Apéry set of the integer 1](Apery-set-integer.qmd)
...
$$
A_1 = \{1, 2, 3\}, A_2 = \{5, 6\}, A_3 = \{9\}, A_4 = \{13\}
$$ On the other hand,
...
Given a numerical semigroup $S$, the function `Deserts` returns a list with all the deserts of $S$.
