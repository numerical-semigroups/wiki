...
Let $S$ be a [numerical semigroup](../Basic-concepts/Numerical-semigroup.qmd), $s \in S$ and $\mathbf{Z}(s)$ its [set of factorizations](../Presentations-NS/Set-of-factorizations.qmd). Is defined the monotone catenary degree of $s$ in $S$ as the least $N$ such that for any two factorizations $x,y \in \mathbf{Z}(s)$ with [lengths](Length-factorization.qmd) $|x| \le |y|$, there exists an [N-chain](Chain-of-factorizations.qmd) of factorizations with non-decreasing lengths joining them.
...
concluding that $AdjC(100) = 5$ and $MaxC(S) = 5$.
