...
$\circ$ Let $S = \langle 10, 11, 17, 23 \rangle$. Since $Betti(S) = \{33, 34, 40, 69 \}$, where $Betti(S)$ denotes the set of [Betti elements](Betti-element.qmd) of $S$, it is deduced that its catenary degree is $\mathbf{C}(S) = 6$, then $\omega(S) \ge 6$. One can also prove that $\mathbf{t}(S) = 6$, concluding that $\omega(S) = 6$.
