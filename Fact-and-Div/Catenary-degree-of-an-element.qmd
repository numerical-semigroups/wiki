...
From this graph, it is deduced that the least $N$ is $N = 5$, then $\mathbf{C}(85) = 5$.
...
Given the set of factorizations of an element, the function `CatenaryDegree` computes the catenary degree.
...
If we give a numerical semigroup $S$ and an element $n$ of it, the function returns the catenary degree of $n$ in $S$.
...
Finally, if we give only a numerical semigroup $S$, the function returns the catenary degree of $S$.
