...
Let $S$ be a [numerical semigroup](./Basic-concepts/Numerical-semigroup.qmd) and $E$ a [proper ideal](Proper-ideal.qmd) of $S$. It is said that $E$ is integrally closed if it is of the form $S(s) = (s + \mathbb{N}) \cap S$ for some $s \in S$.

Note that $E$ is integrally closed if, and only if, $E = S(m(E))$, where $m(E)$ denotes the [multiplicity ideal](Multiplicity-ideal.qmd) of $E$.
