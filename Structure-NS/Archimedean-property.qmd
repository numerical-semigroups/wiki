...
$\circ$ Let $A = \{a_1, \ldots, a_n\}$ be a set of symbols, let $*$ be the concatenation operation and let us consider the semigroup $S = \{ c_1c_2 \cdots c_k ~ | ~ c_i \in A, i \in \{1, 2, \ldots, k\} \}$. No element of $S$ is Archimedean, given $x = a_{i_1}\cdots a_{i_m}$ with $i_1, \ldots, i_m \in \{1,2,\ldots, n\}$, for $y = a_j$ with $j \ne i_1$, there no exist $k \in \mathbb{N} \setminus \{0\}$ and $z \in S$ such that $kx = y * z$.
...
Nowadays, there are no functions in package NumericalSgps related to Archimedean property.
