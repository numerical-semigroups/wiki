...
Nowadays, there are no functions in package NumericalSgps related to Bézout sequence. However, given a list $I = [[a_1, b_1], \ldots, [a_p, b_p]]$ where $a_i, b_i$ are positive integers for all $i \in \{1, \ldots, p\}$, the following function returns true if $\frac{a_1}{b_1} < \ldots < \frac{a_p}{b_p}$ is a Bézout sequence and false otherwise.

``` gap
gap> IsBezoutSeq := function(I)
>       local n;
>       n := Length(I);
>       if n = 0 then
...
>       fi;
>       for i in [1..n-1] do
>               if not I[i+1][1]*I[i][2] - I[i+1][2]*I[i][1] = 1 then
>                        return false;
>               fi;
>       od;
>       return true;
...
$\diamond$ Let $\frac{5}{3} < \frac{12}{7} < \frac{7}{4} < \frac{9}{5}$.

``` gap
gap> IsBezoutSeq([ [5,3], [12, 7], [7,4], [9,5] ]);
true
```

then, the sequence is a Bézout sequence.
