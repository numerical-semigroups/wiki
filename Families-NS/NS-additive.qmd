...
It can be proven that $S$ is an additive numerical semigroup if, and only if, $adj(S) = Ap(B, m)$, where $adj(S)$ is the [adjustment](../Fact-and-div/Adjustment-NS.qmd) of $S$, $B$ is the [blowup](../Ideals-NS/Blowup.qmd) of $S$ and $Ap(B, m)$ is the [Apéry set](../Basic-concepts/Aper-set.qmd) of $B$ in $m$.
