...
>           return false; 
>       fi;
>       list_c := ConstantsFreeNumericalSemigroup(S);
>       return Min_gen[1] - 1 = Sum(I{[2..p]}) - p + 1;
> end;
```

The function `ConstantsFreeNumericalSemigroup` is not implemented in GAP, it computes the constants $c_i$ defined above.

``` gap
gap> ConstantsFreeNumericalSemigroup := function(S)
>       local Min_gen, p, list_c, i, n_i, d_i, S_i, k, q;
>       if not IsNumericalSemigroup(S) then
>           Error("The argument must be a Numerical Semigroup");
>       fi;
>       Min_gen := Set(MinimalGenerators(S));
