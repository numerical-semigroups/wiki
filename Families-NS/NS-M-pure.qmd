...
$$
where $ord(\cdot)$ denotes the [order](Order-of-an-element.qmd) of the respective element in $S$. It is said that $S$ is $M-$pure if the maximal elements of $Ap(S,m)$ with respect to $\preceq_S$ have all the same [order](../Fact-and-div/Order-of-an-element.qmd).
