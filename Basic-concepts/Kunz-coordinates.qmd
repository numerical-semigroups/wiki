...
It is defined the Kunz coordinates of $S$ with respect to $n$ as the tuple $(x_1, \ldots, x_{n-1}) \in \mathbb{N}^n$. If we consider $n = m(S)$, where $m(S)$ denotes the [multiplicity](Multiplicity.qmd) of $S$, it is said directly as the Kunz coordinates of $S$.
...
obtaining that the tuple $(3, 1, 3, 1)$ is the Kunz coordinate of $S$.
...
If we only give a numerical semigroup, the functions computes the Kunz coordinates of $S$.
