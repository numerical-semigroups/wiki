...
Let $S$ be a [numerical semigroup](../Basic-concepts/Numerical-semigroup.qmd) and $n \in S \setminus \{0\}$. If $w(i)$ denotes the smallest element in $S$ such that $w(i) \equiv i ~~ (mod ~ n)$ for all $i \in \{0, \ldots, n-1\}$ and $\overline{m} = m ~ (mod ~ n)$ for all $m \in \mathbb{N}$, it is defined the cocycle of $S$ with respect to $n$ as the matrix $(h_{i,j})_{i, j = 0}^{n-1}$, where $h_{i,j}$ is defined as

$$
h_{i,j} = \frac{w(i) + w(j) - w(\overline{i+j})}{n}.
$$
...
Given a numerical semigroup $S$ and an element $m \in S \setminus \{0\}$, the function `CocycleOfNumericalSemigroupWRTElement` returns the cocycle of $S$ with respect to $m$.
