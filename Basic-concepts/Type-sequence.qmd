...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd), let $G(S) = \{x_1, \ldots, x_g\}$ be the [gaps](Gap.qmd) of $S$ and $N(S) = \{s_1, \ldots, s_n\}$ the set of [left elements](Left-elements.qmd) of $S$, both ordered from smallest to larger. For each $j \in \{1, \ldots, g\}$, it is defined
...
Since $0 \in N(S)$, it follows that $\sigma_j$ is well defined. Moreover, from the definition it is deduced that $t_1(S)$ is the [type](Type.qmd) of $S$ and its sum is $\sum_{i = 1}^n t_i(S) = |G(S)|$, the [genus](Genus.qmd) of $S$.

## Examples

$\circ$ For any numerical semigroup with set of gaps $G(S) = \{x_1, \ldots, x_g\}$ and left elements $N(S) = \{s_1, \ldots, s_n\}$, both ordered from smallest to larger, it holds that $t_i(S) \ne 0$ for all $i \in \{1, \ldots, n\}$. Indeed, for any $i \in \{1, \ldots, n\}$, it is well known that $F(S) - s_i \in G(S)$, where $F(S)$ denotes the [Frobenius number](Frobenius-number.qmd) of $S$, and by definition, it holds that $F(S) - s_i + s_{j} \in S$ for all $j \in \{i+1, \ldots, n\}$. Therefore, taking into account that $F(S) - s_i + s_i = F(S) \not \in S$, if $x_j = F(S) - s_i$, it is concluded that $\sigma_j = i$ and $t_i(S) \ne 0$.

$\circ$ Let $S = \langle 5, 7, 9 \rangle$. Its set of gaps and its set of left elements are respectively $G(S) = \{1, 2, 3, 4, 6, 8, 11, 13\}$ and $N(S) = \{0, 5, 7, 9, 10, 12\}$. With few computations,
