...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd), let $G(S) = \{x_1, \ldots, x_g\}$ be the [gaps](Gaps.qmd) of $S$ and $N(S) = \{s_1, \ldots, s_n\}$ the set of [left elements](Left-elements.qmd) of $S$, both ordered from smallest to larger. For each $j \in \{1, \ldots, g\}$, it is defined
