...
Let $S$ be an [Arf](../Families-NS/Arf-semigroup.qmd) [numerical semigroup](Numerical-semigroup.qmd) and $M = \{m_1, \ldots, m_k\}$ a set of positive integers with $gcd(M) = 1$ and $m_1 < \ldots < m_k$. It is said that $M$ is an Arf system of generators of $S$ if the [Arf closure](Arf-closure.qmd) of $M$ is $S$.
