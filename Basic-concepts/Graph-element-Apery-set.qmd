...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd), $n \in S$ and let $Ap(S,n)$ be the [Apéry set](Apery-set.qmd) of $S$ with respect to $n$. It is defined the graph associated to $n$ by Apéry list as the graph with vertices $Ap(S, n)$ and $(u,v)$ is an edge if, and only if, $v - u \in Ap(S, n)$. The graph is denoted by $(Ap(S,n), E_n)$.

The relation $u \le v \Longleftrightarrow v-u \in Ap(S,n)$ in $Ap(S,n)$ is a partial order, since the conditions $v-u \in S$, $v \in AP(S,n)$ and $u \in S$ implies that $v-u \in Ap(S,n)$.
...
and is well known that $|Ap(S, n)| = n$, where it is deduced that $Ap(S, 7) = \{0, 3, 6, 9, 11, 12, 15\}$. Taking into account that if $(u,v)$ is an edge, then $u \le v$, with few calculations we have that the set of edges is
...
Given a numerical semigroup $S$ and an element $n \in S$, the function `HasseDiagramOfAperyListOfNumericalSemigroup` returns the Hasse diagram by the relation $u \le v$ if, and only if, $v - u \in Ap(S, n)$, with $u,v \in Ap(S, n)$. The function `DotBinaryRelation` return a GraphViz dot that represents the given binary relation.
