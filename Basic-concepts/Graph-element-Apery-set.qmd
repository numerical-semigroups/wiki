...
Given a numerical semigroup $S$ and an element $n \in S$, the function `HasseDiagramOfAperyListOfNumericalSemigroup` returns the Hasse diagram by the relation $u \le v$ if, and only if, $v - u \in Ap(S, n)$, with $u,v \in Ap(S, n)$. The function `DotBinaryRelation` returns a GraphViz dot that represents the given binary relation.
