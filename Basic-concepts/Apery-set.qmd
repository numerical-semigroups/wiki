...
The knowledge of this set determines completely the numerical semigroup $S$. $Ap(S,n)$ always has $n$ elements and it is easy to obtain the [Frobenius number](Frobenius-number.qmd) and the [genus](Genus.qmd) of $S$ from this set. This definition could be extended to the [Apéry set of an integer](../Fact-and-div/Apery-set-integer.qmd) or to the [Apéry set of a set of integers](../Gluing-NS/Apery-set-respect-set.qmd).
