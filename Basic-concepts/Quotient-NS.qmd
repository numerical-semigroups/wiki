---
title: "Quotient of a numerical semigroup"
format: html
engine: markdown
---
...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd) and $p$ be a positive integer. It is defined the quotient of $S$ by $p$, denoted by $\frac{S}{p}$, as

$$
\frac{S}{p} = \{x \in \mathbb{N} ~ | ~ px \in S\}.
$$

It can be proven that $\frac{S}{p}$ is always a numerical semigroup.
...
The function `MinimalGenerators` returns the [minimal generators](Minimal-set-of-generators.qmd) of the given numerical semigroup, then $\frac{S}{2} = \langle 13, 15, 17, 19, 20, 21, 29 \rangle$. It is also possible to generate it with the $/$ operator.
