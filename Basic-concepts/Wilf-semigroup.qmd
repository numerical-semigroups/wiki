...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd) with [embedding dimension](Embedding-dimension.qmd) $e(S)$, $n(S)$ [left elements](Left-elements.qmd) and [conductor](Conductor.qmd) $C(S)$. It is said that $S$ is a Wilf semigroup if $W(S) \ge 0$, where
