...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd) and $PF(S)$ the set of [pseudo-Frobenius numbers](Pseudo-Frobenius-number.qmd) of $S$. An element $x \in PF(S)$ is a special gap if $2x \in S$. The set of special gaps is denoted by $SG(S)$ and is always non-empty, due to $F(S) \in SG(S)$, where $F(S)$ denotes the [Frobenius number](Frobenius-number.qmd) of $S$. Its definition is motivated by the problem of finding the set of all numerical semigroups containing a given numerical semigroup.

## Examples

$\circ$ Consider $m \in \mathbb{N}$ a non-zero element arbitrary but fixed and $S = \{0, m, \rightarrow \}$. Its easy to see that $PF(S) = \{1,2, \ldots, m-1\}$, therefore an element $n \in PF(S)$ is a special gap if $2n \ge m$, that is, if $n \ge \lceil \frac{m}{2} \rceil$, in conclusion, $SG(S) = \{\lceil \frac{m}{2} \rceil, \lceil \frac{m}{2} \rceil + 1, \ldots, m-1 \}$.
...
The function `SpecialGaps` returns the set of special gaps of a numerical semigroup, in this case:
