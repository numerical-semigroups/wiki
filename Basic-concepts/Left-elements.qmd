...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd) and $F(S)$ the [Frobenius number](Frobenius-number.qmd) of $S$. An element $s \in \mathbb{N}$ is a left element (or small element) of $S$ if $s \in S$ and $s < F(S)$. The set of left elements is denoted by $N(S)$ or $L(S)$.
