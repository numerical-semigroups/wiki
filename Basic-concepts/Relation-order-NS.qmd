...
$$ It is easy to prove that $\le_S$ is a partial order relation. From this relation order is possible to characterize the [pseudo-Frobenius numbers](Pseudo-Frobenius-number.qmd) of $S$.
...
$\diamond$ Let $S$ be a numerical semigroup and $F(S)$ the [Frobenius number](Frobenius-number.qmd) of $S$. If we suppose $F(S) \le_S x$ for some $x \in \mathbb{Z} \setminus S$, then exists $s \in S$ such that $x = F(S) + y$, but that means $x \in S$, which is a contradiction, therefore $F(S)$ is maximal in $\mathbb{Z} \setminus S$.
