...
$\diamond$ Let $S$ a numerical semigroup and $F(S)$ the [Frobenius number](Frobenius-number.qmd) of $S$. If we suppose $F(S) \le_S x$ for some $x \in \mathbb{Z} \setminus S$, then exists $s \in S$ such that $x = F(S) + y$, but that means $x \in S$, which is a contradiction, therefore $F(S)$ is maximal in $\mathbb{Z} \setminus S$.
