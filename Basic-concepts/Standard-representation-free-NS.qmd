...
Nowadays, there are no functions in NumericalSgps related to standard representations in a free numerical semigroup. However, given a numerical semigroup $S$, a list $I = \{r_0, \ldots, r_h\}$ such that $S$ is free for the arrangement $(r_0, \ldots, r_h)$ and an integer $z \in \mathbb{Z}$, the following function returns a list with the coefficients of the standard representation of $z$ in $S$.
...
```gap
gap> IdentityBezoutGeneralized := function(I)
>       local list_d, i, list_coef, x, y, prod;
>       n := Length(I):
>       if n = 1 then return [1]; fi;
>       for i in [1..n] do
>           Add(list_d, Gcd(I{[1..i]}));
>       od;
>       list_coef := [];
...
```gap
gap> IdentityBezout := function(a,b)
>       local r_0, r_1, x_0, x_1, x_2, y_0, y_1, y_2, q_2;
>       if a < 0 or b < 0 then
>           Error("The arguments must be positive integers");
>       fi;
>       if a = 0 then return [0,1,b]; fi;
>       if b = 0 or a = b then return [1,0,a]; fi;
>       if a > b then;
>           r_0 := a; r_1 := b;
>           x_0 := 1; x_1 := 0;
>           y_0 := 0; y_1 := 1;
