...
>       local n, list_d, i, j, list_coef, sol, x, y, prod;
>       n := Length(I);
>       list_d := [];
...
>           sol := IdentityBezout(list_d[j-1], I[j]);
>           x := sol[1];
>           y := sol[2];
>           Add(list_coef, y * prod);
>           prod := prod * x;
>       od;
>       Add(list_coef, prod);
>       return Reversed(list_coef);
> end;
```
...
>       local r_0, r_1, r_2, x_0, x_1, x_2, y_0, y_1, y_2, q_2;
>       if a < 0 or b < 0 then
