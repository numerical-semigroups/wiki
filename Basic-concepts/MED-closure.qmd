...
Let $S$ be a [numerical semigroup](Numerical-semigroup.qmd) and let $\mathcal{O}(S)$ be the set of [oversemigroups](Oversemigroup.qmd) of $S$. It is defined the maximal embedding dimension closure (or MED closure) of $S$ as the smallest [MED numerical semigroup](../Families-NS/NS-maximal-embedding-dimension.qmd) containing $S$.
