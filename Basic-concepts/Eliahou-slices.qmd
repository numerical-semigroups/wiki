...
$$
I_j = [jm(S) - \rho(S), (j+1)m(S) - \rho(S)) \cap \mathbb{Z}, ~~~~ \text{for } ~ j \in \{1, \ldots, q(S)\},
$$

it is defined the Eliahou slices of $S$ as the sets

$$
S_j = S \cap I_j ~~~~ \text{for } ~ j \in \{1, \ldots, q(S)\}. 
$$
...
$$
I_j = [7j - 1, 7(j+1) - 1) \cap \mathbb{Z}, ~~~~ \text{for } ~ j = 1, 2, 3.
$$
...
Given a numerical semigroup $S$, the function `EliahouSlicesOfNumericalSemigroup` returns a list with the Eliahou slices of $S$ for $j = 1, \ldots, q(S)-1$.
...
In order to obtain $S_q = S \cap I_q$, the following functions computes it. The interval $S_q$ is called the [threshold interval](Threshold%20interval) of $S$.
